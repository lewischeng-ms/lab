...
namespace Microsoft.AspNet.OData.Query.Expressions
{
    ...
    internal class FilterBinder
    {
        ...
        private FilterBinder(IEdmModel model, IAssemblyProvider assembliesResolver, ODataQuerySettings querySettings)
            : this(model, querySettings)
        {
            _assemblyProvider = assembliesResolver;
        }
        ...
        public static Expression<Func<TEntityType, bool>> Bind<TEntityType>(FilterClause filterClause, IEdmModel model,
            IAssemblyProvider assembliesResolver, ODataQuerySettings querySettings)
        {
            ...
            IAssemblyProvider assembliesResolver, ODataQuerySettings querySettings)
        {
            ...
            {
                throw Error.Argument("filterType", "Cannot cast from '{0}' to '{1}'", filter.Type.FullName, expectedFilterType.FullName);
            }
        ...
        private Expression Bind(QueryNode node)
        {
            ...
            if (collectionNode != null)
            {
                switch (node.Kind)
                {
                    ...
                    default:
                        throw Error.NotSupported("'{1}' does not support '{0}'", node.Kind, typeof(FilterBinder).Name);
                }
            }
        ...
        private Expression BindSingleEntityFunctionCallNode(SingleEntityFunctionCallNode node)
        {
            switch (node.Name)
            {
                ...
                default:
                    throw Error.NotSupported("Function '{0}' not supported", node.Name);
            }
        }
        ...
        private Expression BindSingleEntityCastFunctionCall(SingleEntityFunctionCallNode node)
        {
            ...
            {
                targetClrType = EdmLibHelpers.GetClrType(targetEdmType.ToEdmTypeReference(false), _model, _assemblyProvider);
            }
        ...
        private Expression BindSingleEntityCastNode(SingleEntityCastNode node)
        {
            ...
            Type clrType = EdmLibHelpers.GetClrType(entity, _model, _assemblyProvider);
        ...
        private Expression BindUnaryOperatorNode(UnaryOperatorNode unaryOperatorNode)
        {
            ...
            switch (unaryOperatorNode.OperatorKind)
            {
                ...
                default:
                    throw Error.NotSupported("'{1}' does not support '{0}'", unaryOperatorNode.Kind, typeof(FilterBinder).Name);
            }
        }
        ...
        private Expression BindSingleValueFunctionCallNode(SingleValueFunctionCallNode node)
        {
            switch (node.Name)
            {
                ...
                default:
                    throw new NotImplementedException(Error.Format("Function '{0}' not supported", node.Name));
            }
        }
        ...
        private Expression BindCastSingleValue(SingleValueFunctionCallNode node)
        {
            ...
            if (targetEdmType != null)
            {
                ...
                targetClrType = EdmLibHelpers.GetClrType(targetEdmTypeReference, _model, _assemblyProvider);
        ...
        private Expression BindSubstring(SingleValueFunctionCallNode node)
        {
            ...
            {
                throw new ODataException(Error.Format("Function '{0}' not supported on enum", node.Name));
            }
        ...
        private static void ValidateAllStringArguments(string functionName, Expression[] arguments)
        {
            ...
            {
                throw new ODataException(Error.Format("Function '{0}' not supported on enum", functionName));
            }
        }
        ...
        private Expression ConvertNonStandardPrimitives(Expression source)
        {
            ...
            Type conversionType = EdmLibHelpers.IsNonstandardEdmPrimitive(source.Type, out isNonstandardEdmPrimitive, _assemblyProvider);
            ...
            if (isNonstandardEdmPrimitive)
            {
                ...
                else
                {
                    ...
                    {
                        convertedExpression = Expression.Convert(ExtractValueFromNullableExpression(source),
                            conversionType);
                    }
                    else if (sourceType == typeof (char))
                    {
                        convertedExpression = Expression.Call(ExtractValueFromNullableExpression(source), "ToString",
                    ...
                    {
                        convertedExpression = Expression.New(
                            typeof (string).GetConstructor(new[] {typeof (char[])}), source);
                    }
                    else if (sourceType == typeof (XElement))
                    {
                        convertedExpression = Expression.Call(source, "ToString", typeArguments: null,
                            arguments: null);
                    }
                    ...
                    else
                    {
                        Contract.Assert(false,
                            Error.Format("missing non-standard type support for {0}", sourceType.Name));
                    }
                }
        ...
        private Expression CreateBinaryExpression(BinaryOperatorKind binaryOperator, Expression left, Expression right, bool liftToNull)
        {
                ...
                if (left.Type == typeof(byte[]) || right.Type == typeof(byte[]))
                {
                    ...
                    switch (binaryExpressionType)
                    {
                        ...
                        default:
                            ...
                            throw new ODataException(Error.Format("Binary operator '{1}' not supported for type '{0}'", binaryType.FullName(), binaryOperator));
                    }
                }
                else
                {
            ...
            else
            {
                ...
                {
                    throw Error.NotSupported("'{1}' does not support '{0}'", binaryOperator, typeof(FilterBinder).Name);
                }
            }
        }
        ...
        private static Expression ConvertToEnumUnderlyingType(Expression expression, Type enumType, Type enumUnderlyingType)
        {
            ...
            {
                throw Error.NotSupported("Cannot convert '{1}' to enum '{0}'", enumType, expression.Type);
            }
        }
